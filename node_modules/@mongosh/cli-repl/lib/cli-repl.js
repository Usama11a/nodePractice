...
class CliRepl {
    constructor(options) {
        var _a, _b;
        this.config = new types_1.CliUserConfig();
    ...
    async start(driverUri, driverOptions) {
        ...
        setup_logger_and_telemetry_1.default(this.logId, this.bus, () => pino_1.default({ name: 'mongosh' }, logStream), () => {
            ...
            this.analytics = new analytics_node_1.default((_c = (_b = this.analyticsOptions) === null || _b === void 0 ? void 0 : _b.apiKey) !== null && _c !== void 0 ? _c : require('./analytics-config.js').SEGMENT_API_KEY, this.analyticsOptions);
            return this.analytics;
        });
        this.config.userId = new service_provider_core_1.bson.ObjectId().toString();
    ...
    async openLogStream() {
        ...
        try {
            const stream = fs_1.createWriteStream(path, { mode: 0o600 });
    ...
    async connect(driverUri, driverOptions) {
        if (!this.cliOptions.nodb && !this.cliOptions.quiet) {
            this.output.write(i18n_1.default.__(CONNECTING) + '\t\t' + this.clr(history_1.redactCredentials(driverUri), ['bold', 'green']) + '\n');
        }
        const provider = await service_provider_server_1.CliServiceProvider.connect(driverUri, driverOptions, this.cliOptions);
