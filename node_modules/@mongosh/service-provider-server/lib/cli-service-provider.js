...
class CliServiceProvider extends service_provider_core_1.ServiceProviderCore {
    ...
    static async connect(uri, driverOptions = {}, cliOptions = {}) {
        const connectionString = new service_provider_core_1.ConnectionString(uri || 'mongodb://nodb/');
        const clientOptions = processDriverOptions(driverOptions);
        const mongoClient = !cliOptions.nodb ?
            await connectMongoClient(connectionString.toString(), clientOptions) :
    ...
    async getNewConnection(uri, options = {}) {
        const connectionString = new service_provider_core_1.ConnectionString(uri);
        const clientOptions = processDriverOptions(options);
        const mongoClient = await connectMongoClient(connectionString.toString(), clientOptions);
